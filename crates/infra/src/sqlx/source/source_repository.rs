use application::source::repository::SourceRepository;
...
fn to_database_error(error: sqlx::error::Error) -> application::source::repository::Error {
  application::source::repository::Error::DatabaseError(error.to_string())
}
...
impl SourceRepository for SqlxSourceRepository {
  async fn create(
    &mut self,
    source: &Source,
  ) -> Result<(), application::source::repository::Error> {
    sqlx::query!(
      r#"
  ...
  async fn find_by_id(
    &mut self,
    id: &UUID4,
  ) -> Result<Option<Source>, application::source::repository::Error> {
