use ::sqlx;
use application::media::repository::{FindByQuery, MediaRepository};
use domain::{
  artist::entity::Artist,
  media::{
    aggregate::MediaAggregate,
    entity::{Media, MediaBuilder},
  },
...
use crate::{create_filter, search_by, sqlx::util::many_to_many::ManyToManyBuilder};
...
fn to_database_error(error: sqlx::error::Error) -> application::media::repository::Error {
  application::media::repository::Error::DatabaseError(error.to_string())
}

impl MediaRepository for SqlxMediaRepository {
  async fn create(&mut self, media: &Media) -> Result<(), application::media::repository::Error> {
  ...
  async fn update(&mut self, media: &Media) -> Result<(), application::media::repository::Error> {
    let mut trx = self.db.begin().await.map_err(to_database_error)?;
  ...
  async fn find_by_id(
    &mut self,
    id: &UUID4,
  ) -> Result<Option<Media>, application::media::repository::Error> {
    ...
    .await
    .map_err(to_database_error)?
    .map(|value| Media::builder()
    .id(UUID4::from(value.id))
    .name(value.name)
    .media_type(value.media_type.parse().unwrap())
    .slug(Slug::new(value.slug).unwrap())
    .cover(value.cover)
      ...
      async {
        ...
        .map(|result| {
          result
            .into_iter()
            .map(UUID4::from)
            .collect::<std::collections::HashSet<UUID4>>()
        })
      },
      async {
        sqlx::query_scalar!(
  ...
  async fn delete_by_id(
    &mut self,
    id: &UUID4,
  ) -> Result<(), application::media::repository::Error> {
    sqlx::query!(
  ...
  async fn find_by(
    &mut self,
    query: &FindByQuery,
  ) -> Result<Paged<MediaAggregate>, application::media::repository::Error> {
    let filter = create_filter!(FindByQuery, qb => {
              ...
              .map(|media| {
                ...
                async move {
                  ...
                  Ok(MediaAggregate {
                    ...
                    as Result<MediaAggregate, application::media::repository::Error>
                }
              })
              .collect::<Vec<_>>()
          })
