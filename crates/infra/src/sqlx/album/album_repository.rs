...
use application::album::repository::{AlbumRepository, FindAllQuery};
use domain::{
  album::{aggregate::AlbumAggregate, entity::Album},
...
impl AlbumRepository for SqlxAlbumRepository {
  async fn create(
    &mut self,
    album: &Album,
  ) -> Result<Album, application::album::repository::Error> {
    let mut trx = self
      .db
      .begin()
      .await
      .map_err(|error| application::album::repository::Error::DatabaseError(error.to_string()))?;
    ...
    .execute(&mut *trx)
    .await
    .map_err(|err| application::album::repository::Error::DatabaseError(format!("Falha ao associar artistas: {}", err)))?;
    ...
    ManyToManyBuilder::new()
      ...
      .await
      .map_err(|err| application::album::repository::Error::DatabaseError(err.to_string()))?;
  ...
  async fn find_by_id(
    &mut self,
    id: &UUID4,
  ) -> Result<Option<Album>, application::album::repository::Error> {
  ...
  async fn delete_by_id(
    &mut self,
    id: &UUID4,
  ) -> Result<(), application::album::repository::Error> {
    let mut trx = self
      .db
      .begin()
      .await
  ...
  ) -> Result<Option<AlbumAggregate>, application::album::repository::Error> {
    let result = sqlx::query!(
      r#"
      select "a"."id", 
      "a"."name",
  ...
  async fn find_by(
    &mut self,
    query: &FindAllQuery,
  ) -> Result<Paged<AlbumAggregate>, application::album::repository::Error> {
    let filter = create_filter!(FindAllQuery, qb => {
